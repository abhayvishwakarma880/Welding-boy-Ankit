...
import { getBlogBySlugOrId, getBlogs, addComment } from "@/apis/blogs";
import { CalendarDays, Clock, ChevronRight, Search, Share2 } from "lucide-react";
...
export default function BlogDetailPage() {
  ...
  const [recentBlogs, setRecentBlogs] = useState<RelatedBlog[]>([]);

  useEffect(() => {
    setLoading(true);
    getBlogBySlugOrId(slug)
  ...
  if (!blog) return (
    <div className="min-h-screen flex items-center justify-center">
      <p className="text-slate-500 text-sm">Blog not found.</p>
    </div>
  );

  const relatedToShow = blog.relatedBlogs?.length ? blog.relatedBlogs : recentBlogs.filter((b) => b._id !== blog._id).slice(0, 4);
          ...
          <div className="flex-1 min-w-0">
            ...
            {blog.tags?.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-8 pt-6 border-t border-slate-100">
                ...
                {blog.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-600 hover:bg-brand/10 hover:text-brand transition-colors cursor-pointer">
                    #{tag}
                  </span>
                ))}
              </div>
                ...
                <div className="mt-3 space-y-2">
                  {searching ? (
                    ...
                    searchResults.map((b) => (
                      ...
                      >
                        <img src={b.image?.url} alt={b.title} className="w-10 h-10 rounded-lg object-cover shrink-0" />
                        <p className="text-xs font-medium text-slate-700 line-clamp-2 group-hover:text-brand transition-colors">{b.title}</p>
                      </Link>
                    ))
                  ) : (
...
function CommentSection({ blogId }: { blogId: string }) {
  ...
  return (
    <div className="mt-8 pt-6 border-t border-slate-100">
      <h3 className="text-base font-bold text-slate-800 mb-4">Post a Comment</h3>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          {submitted && (
            <span className="text-xs text-green-600 font-medium">Comment posted successfully!</span>
          )}
          <button
            type="submit"
